...
> Modules类里面的方法专门提供依赖，所以我们定义一个类，用@Module注解，Dagger在构造类的实例的时候，会根据返回值类型
##2.2 @Provides
...
>对于不同的对象有同样的返回类型，我们可以使用@Named修饰符注解。你需要在提供单例的地方(@Provides注解)和注入的地方（@Inject注解）都使用@Named注解。这一点类似于Java的多态。